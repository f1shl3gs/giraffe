...
export interface PaginationNavProps extends StandardFunctionProps {
  /** Total number of pages there exists */
  totalPages: number
...
export const Pagination: FunctionComponent<PaginationNavProps> = ({
  id,
  style,
  ...
  return (
    ...
    >
      <ul className={'cf-pagination--container'} ref={innerRef}>
        ...
        {[...Array(totalPages)]
          .map((_, i) => i)
          .slice(breakpoints.firstBreakpoint, breakpoints.secondBreakpoint + 1)
          .map(item => (
            <PaginationItem
              page={item.toString()}
